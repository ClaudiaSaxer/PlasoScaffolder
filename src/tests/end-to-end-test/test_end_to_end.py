  ...
  def testEasyGeneration(self):
    ...
    if platform.system() in ['Linux']:
      with tempfile.TemporaryDirectory() as tmpdir:
        ...
        child.expect('create.*{0}'.format(formatter_path))
        child.expect('create.*{0}'.format(parser_path))
        child.expect('create.*{0}'.format(formatter_test_path))
        child.expect('create.*{0}'.format(parser_test_path))
        child.expect('copy.*{0}'.format(test_data_path))
        child.expect('create.*{0}'.format(parsers_init_path))
        child.expect('create.*{0}'.format(formatter_init_path))
