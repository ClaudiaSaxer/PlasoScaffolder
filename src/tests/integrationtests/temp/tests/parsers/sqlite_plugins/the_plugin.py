...
class ThePluginTest(test_lib.SQLitePluginTestCase):
  ...
  def testProcess(self):
    ...
    expected_advertiser_account_type = (
        u'0')
    expected_analytics_type = (
        u'0')
    expected_bio_entities = (
        u'None')
    expected_business_profile_state = (
        u'0')
    expected_could_be_stale = (
        u'0')
    ...
    expected_device_following = (
        u'0')
    expected_extended_profile_fields = (
        u'None')
    expected_favorites_count = (
        u'0')
    expected_followers_count = (
        u'19466932')
    expected_followers_count_fast = (
        u'0')
    expected_followers_count_normal = (
        u'0')
    expected_following = (
        u'0')
    expected_following_count = (
        u'3')
    expected_has_collections = (
        u'0')
    expected_has_extended_profile_fields = (
        u'0')
    expected_id = (
        u'5402612')
    expected_is_lifeline_institution = (
        u'0')
    ...
    expected_profile_image_url = (
        ...
        u'_normal.png')
    expected_profile_link_color_hex_triplet = (
        u'2052731')
    expected_protected = (
        u'0')
    ...
    self.assertEqual(test_event.advertiser_account_type, expected_advertiser_account_type)
    self.assertEqual(test_event.analytics_type, expected_analytics_type)
    self.assertEqual(test_event.bio_entities, expected_bio_entities)
    self.assertEqual(test_event.business_profile_state, expected_business_profile_state)
    self.assertEqual(test_event.could_be_stale, expected_could_be_stale)
    self.assertEqual(test_event.description, expected_description)
    self.assertEqual(test_event.device_following, expected_device_following)
    self.assertEqual(test_event.extended_profile_fields, expected_extended_profile_fields)
    self.assertEqual(test_event.favorites_count, expected_favorites_count)
    self.assertEqual(test_event.followers_count, expected_followers_count)
    self.assertEqual(test_event.followers_count_fast, expected_followers_count_fast)
    self.assertEqual(test_event.followers_count_normal, expected_followers_count_normal)
    self.assertEqual(test_event.following, expected_following)
    self.assertEqual(test_event.following_count, expected_following_count)
    self.assertEqual(test_event.has_collections, expected_has_collections)
    self.assertEqual(test_event.has_extended_profile_fields, expected_has_extended_profile_fields)
    self.assertEqual(test_event.id, expected_id)
    self.assertEqual(test_event.is_lifeline_institution, expected_is_lifeline_institution)
    self.assertEqual(test_event.is_translator, expected_is_translator)
    self.assertEqual(test_event.location, expected_location)
    self.assertEqual(test_event.media_count, expected_media_count)
    self.assertEqual(test_event.name, expected_name)
    self.assertEqual(test_event.pinned_tweet_id, expected_pinned_tweet_id)
    ...
    self.assertEqual(test_event.profile_link_color_hex_triplet, expected_profile_link_color_hex_triplet)
    self.assertEqual(test_event.protected, expected_protected)
    self.assertEqual(test_event.screen_name, expected_screen_name)
    self.assertEqual(test_event.statuses_count, expected_statuses_count)
    self.assertEqual(test_event.structured_location, expected_structured_location)
    self.assertEqual(test_event.url, expected_url)
    self.assertEqual(test_event.url_entities, expected_url_entities)
    self.assertEqual(test_event.verified, expected_verified)
    ...
    expected_extra_scribe_item = (
        u'None')
    expected_favorite_count = (
        u'3')
    expected_favorited = (
        u'0')
    expected_full_text_length = (
        u'0')
    expected_geotag = (
        u'None')
    expected_id = (
        u'510031570397577216')
    expected_include_in_profile_timeline = (
        u'1')
    expected_in_reply_to_status_id = (
        u'None')
    expected_in_reply_to_username = (
        u'None')
    expected_is_lifeline_alert = (
        u'0')
    expected_is_possibly_sensitive_appealable = (
        u'0')
    expected_is_truncated = (
        u'0')
    expected_lang = (
        u'en')
    expected_possibly_sensitive = (
        u'0')
    expected_preview_length = (
        u'0')
    expected_primary_card_type = (
        u'0')
    expected_quoted_status_id = (
        u'None')
    expected_retweet_count = (
        u'2')
    expected_retweeted_status_id = (
        u'None')
    expected_source = (
        u'&lt;a href=&#34;http://twitter.com/download/iphone&#34;'
        u'rel=&#34;nofollow&#34;&gt;Twitter for iPhone&lt;/a&gt;')
    expected_supplmental_language = (
        u'None')
    expected_text = (
        u'Never forget. http://t.co/L7bjWue1A2')
    expected_user_id = (
        u'475222380')
    expected_withheld_in_countries = (
        u'None')
    expected_withheld_scope = (
        u'None')
    
    self.assertEqual(test_event.card, expected_card)
    self.assertEqual(test_event.card_users, expected_card_users)
    self.assertEqual(test_event.card_version, expected_card_version)
    self.assertEqual(test_event.entities, expected_entities)
    self.assertEqual(test_event.extra_scribe_item, expected_extra_scribe_item)
    self.assertEqual(test_event.favorite_count, expected_favorite_count)
    self.assertEqual(test_event.favorited, expected_favorited)
    self.assertEqual(test_event.full_text_length, expected_full_text_length)
    self.assertEqual(test_event.geotag, expected_geotag)
    self.assertEqual(test_event.id, expected_id)
    self.assertEqual(test_event.include_in_profile_timeline, expected_include_in_profile_timeline)
    self.assertEqual(test_event.in_reply_to_status_id, expected_in_reply_to_status_id)
    self.assertEqual(test_event.in_reply_to_username, expected_in_reply_to_username)
    self.assertEqual(test_event.is_lifeline_alert, expected_is_lifeline_alert)
    self.assertEqual(test_event.is_possibly_sensitive_appealable, expected_is_possibly_sensitive_appealable)
    self.assertEqual(test_event.is_truncated, expected_is_truncated)
    self.assertEqual(test_event.lang, expected_lang)
    self.assertEqual(test_event.possibly_sensitive, expected_possibly_sensitive)
    self.assertEqual(test_event.preview_length, expected_preview_length)
    self.assertEqual(test_event.primary_card_type, expected_primary_card_type)
    self.assertEqual(test_event.quoted_status_id, expected_quoted_status_id)
    self.assertEqual(test_event.retweet_count, expected_retweet_count)
    self.assertEqual(test_event.retweeted_status_id, expected_retweeted_status_id)
    self.assertEqual(test_event.source, expected_source)
    self.assertEqual(test_event.supplmental_language, expected_supplmental_language)
    self.assertEqual(test_event.text, expected_text)
    self.assertEqual(test_event.user_id, expected_user_id)
    self.assertEqual(test_event.withheld_in_countries, expected_withheld_in_countries)
    self.assertEqual(test_event.withheld_scope, expected_withheld_scope)
