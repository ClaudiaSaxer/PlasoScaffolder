...
class ThePluginUsersFormatter(interface.ConditionalEventFormatter):
  ...
  FORMAT_STRING_PIECES = [
    ...
    u'Analytics Type:{analytics_type}', u'Bio Entities:{bio_entities}',
    u'Business Profile State:{business_profile_state}',
    u'Could Be Stale:{could_be_stale}', u'Description:{description}',
    u'Device Following:{device_following}',
    u'Extended Profile Fields:{extended_profile_fields}',
    u'Favorites Count:{favorites_count}', u'Followers Count:{followers_count}',
    u'Followers Count Fast:{followers_count_fast}',
    u'Followers Count Normal:{followers_count_normal}',
    u'Following:{following}', u'Following Count:{following_count}',
    u'Has Collections:{has_collections}',
    u'Has Extended Profile Fields:{has_extended_profile_fields}', u'Id:{id}',
    u'Is Lifeline Institution:{is_lifeline_institution}',
    u'Is Translator:{is_translator}', u'Location:{location}',
    u'Media Count:{media_count}', u'Name:{name}',
    ...
    u'Protected:{protected}', u'Screen Name:{screen_name}',
    u'Statuses Count:{statuses_count}',
    u'Structured Location:{structured_location}', u'Url:{url}',
    u'Url Entities:{url_entities}', u'Verified:{verified}'
  ]
...
class ThePluginStatusesFormatter(interface.ConditionalEventFormatter):
  ...
  """Correct Format String Pieces where needed"""
  FORMAT_STRING_PIECES = [
    u'Card:{card}', u'Card Users:{card_users}', u'Card Version:{card_version}',
    u'Entities:{entities}', u'Extra Scribe Item:{extra_scribe_item}',
    u'Favorite Count:{favorite_count}', u'Favorited:{favorited}',
    u'Full Text Length:{full_text_length}', u'Geotag:{geotag}', u'Id:{id}',
    ...
    u'Is Truncated:{is_truncated}', u'Lang:{lang}',
    u'Possibly Sensitive:{possibly_sensitive}',
    u'Preview Length:{preview_length}',
    u'Primary Card Type:{primary_card_type}',
    u'Quoted Status Id:{quoted_status_id}', u'Retweet Count:{retweet_count}',
    u'Retweeted Status Id:{retweeted_status_id}', u'Source:{source}',
    u'Supplmental Language:{supplmental_language}', u'Text:{text}',
    u'User Id:{user_id}', u'Withheld In Countries:{withheld_in_countries}',
