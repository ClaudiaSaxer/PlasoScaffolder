...
def _CheckPythonModule(
  ...
  if maximum_version:
    ...
    if module_version_map > maximum_version_map:
      print((
              u'[FAILURE]\t{0:s} version: {1!s} is too recent, {2!s} or '
              u'earlier '
              u'required.').format(module_name, module_version,
                                   maximum_version))
      return False
...
def CheckModuleVersion(module_name):
  ...
  if module_version_map < minimum_version_map:
    raise ImportError((
                        u'Module: {0:s} version: {1!s} is too old, {2!s} or '
                        u'later '
                        u'required.').format(module_name, module_version,
                                             minimum_version))
  ...
  if maximum_version:
    ...
    if module_version_map > maximum_version_map:
      raise ImportError((
                          u'Module; {0:s} version: {1!s} is too recent, '
                          u'{2!s} or earlier '
                          u'required.').format(module_name, module_version,
                                               maximum_version))
